...
public class MissileBasic : MonoBehaviour {
    ...
    void FindTarget () {
        ...
        var hitColliders = Physics.OverlapCapsule(transform.position - new Vector3(0, 25, 0), transform.position + new Vector3(0, 35, 0), 20).ToList();
        foreach (var target in hitColliders) {
            ...
            for (var i = 0; i < _enemies.Count; i++) {
                if (!hitColliders.Contains(_enemies[i].GetComponent<Collider>())) {
            ...
            if (target.CompareTag("Enemy")) {
                ...
                switch (_mode) {
                    ...
                    case 2:
                        ...
                        if (_enemies.Count > 1) {
                            var farthest = _enemies[0];
                            foreach (var fTar in _enemies) {
                                if (Vector3.Distance(fTar.transform.position, transform.position) >
                                    Vector3.Distance(farthest.transform.position, transform.position)) {
                                    farthest = fTar;
                                }
                            }
                            _target = farthest;
