...
public partial class ExtendedInspector : EditorWindow
{
...
#else
                    EditorUtility.ResetToPrefabState(Selection.activeGameObject);
#endif
    ...
    private void DropDown()
    {
        ...
        if (_dropDownIndex == 0) return;
        if (_dropDownIndex > components.Count) return; // Just stops the error when scripts rebuild when debugging.
    ...
    private void CreateField(object component, InspectorInfo field)
    {
        ...
        else
        {
            ...
            if (v != null) field.SetValue(component, v);
        }
    }
    private void CreateArrayGroup(object component, InspectorInfo field)
    {
        if (!_enabledFoldout[field]) return;
    ...
    private void CreateGenericGroup(object component, InspectorInfo field, Type type)
    {
        ...
        else if (type.GetGenericTypeDefinition() == typeof(Dictionary<,>))
        {
            ...
            try
            {
                ...
                if (_key == null) _key = DefaultValue(arrayType[0]);
                if (_value == null) _value = DefaultValue(arrayType[1]);
